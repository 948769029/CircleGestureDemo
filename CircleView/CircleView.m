...
-(void)showImage
{
    ...
    DragButton *dragImageView = [arrImages objectAtIndex:0];
    ...
    for (int i=0; i<arrImages.count; i++) {
        ...
        DragButton *imageview = [arrImages objectAtIndex:i];
...
- (void)reviseCirclePoint{
    ...
    DragButton *imageviewFirst = [arrImages objectAtIndex:0];
...
- (void)animateWithDuration:(CGFloat)time animateDelay:(CGFloat)delay changeIndex:(NSInteger)change_index toIndex:(NSInteger)to_index circleArray:(NSMutableArray *)array clockwise:(BOOL)is_clockwise{
    DragButton *change_cell = [array objectAtIndex:change_index];
    DragButton *to_cell     = [array objectAtIndex:to_index];
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
    for (int i = 0; i < arrImages.count; ++i)
    {
        ...
        DragButton *change_cell = [arrImages objectAtIndex:i];
        
        DragButton *to_cell     = [arrImages objectAtIndex:iDest];
